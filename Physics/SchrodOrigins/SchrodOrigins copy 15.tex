...
\CR{We explore the transition from a purely classical theory governed by a non-linear classical wave equation to quantum theory governed by Schr\"{o}dinger's equation.  The classical wave equation includes a non-linear classicality enforcing potential.  It is not necessary to completely eliminate this non-linearity to recover linear quantum behavior.  Scaling the classicality enforcing potential is sufficient to have quantum-like features to appear and is equivalent to scaling Planck's constant.}
...
\caption{The red solid line is the analytic probability density using the Schr\"{o}dinger equation with a scaled $\hbar = \tilde{\hbar} \sqrt{\epsilon}$, Eq.~(\ref{eqn:scaled_schrod}), and the black dotted line is the simulated probability density using the transition equation, Eq.~(\ref{eqn:class_schrod_ep}).  All plots are evaluated at the same time $t = 20 m \sigma^2 / \hbar$ and the initial distance from the origin to the center of the two gaussians is $d = 3 \sigma$. Plot (a) is the fully classical case with $\epsilon = 0$. (b) $\epsilon = 0.02$. (c) $\epsilon = 0.05$. (d) $\epsilon = 0.2$. (e) $\epsilon = 0.6$. (f) is for the fully quantum case $\epsilon = 1$ when $\tilde{\hbar} = \hbar$.}
...
%\CR{The transition equation may be an appropriate equation to describe systems in which non-linearities exist and yet have linear quantum-like features. Sbitnev~\cite{bib:navstoke} arrived at the scaled Schr\"{o}dinger equation, Eq.~(\ref{eqn:scaled_schrod}), when he derived a theoretical model of a bouncing droplet system~\cite{bib:dropsdiffract}, a macroscopic system which exhibits quantum-like behavior.  He manipulates the classical Navier-Stokes equation to derive an equation similar in form to the classical  Schr\"{o}dinger-like equation, Eq.~(\ref{eqn:class_schrod}).  He then makes an approximation that removes the non-linear term and ends up with an equation equivalent to the Schr\"{o}dinger equation with a Planck's constant replaced by a macroscopic equivalent which he uses to describe the bouncing droplet system.  This equivalent Planck's constant is comparable to Planck's constant scaled by an appropriate degree of quantumness and as such the bouncing droplet system may also be described using the transition equation.}

\CR{The transition equation may be an appropriate equation to describe systems in which non-linearities exist and yet have linear quantum-like features.  Recently Sbitnev~\cite{bib:navstoke} derived a theoretical model of a bouncing droplet system~\cite{bib:dropsdiffract}, a macroscopic system which exhibits quantum-like behavior.  He manipulates the classical Navier-Stokes equation to derive an equation similar in form to the classical  Schr\"{o}dinger-like equation, Eq.~(\ref{eqn:class_schrod}).  He then makes an approximation that removes the non-linear term and ends up with a governing equation equivalent to the Schr\"{o}dinger equation with a Planck's constant replaced by a macroscopic equivalent which he uses to describe the bouncing droplet system.  This equivalent Planck's constant is comparable to Planck's constant scaled by an appropriate degree of quantumness and as such Sbitnev's governing equation is comparable to the scaled Schr\"{o}dinger equation, Eq.~(\ref{eqn:scaled_schrod}), and the transition equation, Eq.~(\ref{eqn:class_schrod_ep}).}

\PS{For a more general point of view, it may not be necessary to explicitly eliminate the lon-linearity [e.g. through a linearization procedure] in order to encounter linear behavior, related to wave interference and the superposition principle, in the framework of a non-linear theory.  This insight might be of relevance also for other fields of physics that are governed by an intrinsically non-linear equation, such as fluid dynamics.}
